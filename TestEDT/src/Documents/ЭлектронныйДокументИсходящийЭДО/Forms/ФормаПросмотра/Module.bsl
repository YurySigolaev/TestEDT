...
Процедура ДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ДоговокОбработкаВыбора", ЭтотОбъект);
...
&НаКлиенте
Процедура ДоговокОбработкаВыбора(ВыбранноеЗначение, Контекст = Неопределено) Экспорт
...
Процедура ЗаписатьДокумент()
	...
	Если Не ЗначениеЗаполнено(Объект.МаршрутПодписания) 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, Объект.Ссылка, "МаршрутПодписания", "Объект",);
...
Процедура Печать(Команда)
	...
	Если ЭлементСхемы.ТипЭлементаРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя") Тогда
		ДанныеПолучателя = ИнтерфейсДокументовЭДОКлиентСервер.НайтиЭлементСхемыИнформацияПолучателя(ЭлементСхемы);
		Если ДанныеПолучателя <> Неопределено Тогда
			ИнтерфейсДокументовЭДОКлиент.ПечатьЭлектронногоДокумента(ЭлементСхемы.Сообщение, ДанныеПолучателя.Сообщение);
		
		Иначе
			ИнтерфейсДокументовЭДОКлиент.ПечатьЭлектронногоДокумента(ЭлементСхемы.Сообщение);
		Конецесли;
	Иначе	
		ИнтерфейсДокументовЭДОКлиент.ПечатьЭлектронногоДокумента(ЭлементСхемы.Сообщение);
	КонецЕсли;
...
Процедура ПодписатьИОтправитьОбработка()
	...
	ПроверитьЗаписьДокумнетаПередПодписанием(Оповещение);
...
Процедура ПодписатьОтправитьПослеЗаписи(Результат, ПакетнаяОбработка) Экспорт
	...
	Если Модифицированность Тогда
		...
		Если РезультатПроверки.ФормированиеЗапрещено Тогда			
			ОткрытьПомошникСОшибкойЗапретаФормирования(РезультатПроверки);			
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьЗаписьДокумнетаПередПодписанием(Оповещение)
...
&НаКлиенте
Процедура ОтправитьДокументОбработка()
...
&НаКлиенте
Процедура СохранитьДокументВФайл(Команда)
	
	ЭлементСхемы = СхемаРегламента.НайтиПоИдентификатору(ИдентификаторТекущегоЭлемента);

	ИнтерфейсДокументовЭДОКлиент.СозранитьЭлектронныйДокументВФайл(ЭлементСхемы.ПрисоединенныйФайл, УникальныйИдентификатор);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ЗаполнитьТекстСоспроводительнойЗаписки(ЭлементСхемы);
...
Процедура ЗаполнитьДанныеЭлектронногоДокумента()
	...
	ИначеЕсли Параметры.Свойство("РезультатПодготовкиДанных") Тогда
		Если Параметры.НастройкиФормирования.Направление = Перечисления.НаправленияЭДО.Внутренний Тогда
			ЗаполнитьДанныеПредварительногоВнутреннегоДокумента(Параметры.НастройкиФормирования.НастройкиВнутреннегоЭДО,
				Параметры.РезультатПодготовкиДанных);
		Иначе
			ЗаполнитьДанныеПредварительногоЭлектронногоДокумента(Параметры.НастройкиФормирования.НастройкиОтправки,
				Параметры.РезультатПодготовкиДанных);
		КонецЕсли;
	Иначе
...
Процедура НастроитьОформлениеФормы(ЭлементСхемы)
	...
	Элементы.ОткрытьНастройкиФормирования.Видимость = (Объект.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Или
		Объект.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском)
	...
	Иначе
		...
		Элементы.МаршрутПодписанияДокумента.Видимость = Не ЗначениеЗаполнено(Объект.Ссылка) И 
			(Объект.МаршрутПодписания = МаршрутыПодписанияБЭД.МаршрутУказыватьПриСоздании() Или Объект.МаршрутПодписания.Пустая());
	КонецЕсли;
...
Процедура НастроитьВидимостьИНаименованиеДействийПоСостояниюЭДО()
	...
	Элементы.СохранитьДокументВФайл.Видимость = ЕстьПравоОбработкиДокументов;
...
Процедура ПоказатьПредставлениеДокумента(ЭлементСхемыРегламента)
	...
	Если Не ЭлементСхемыРегламента.Распознан И 
		ЭлементСхемыРегламента.Направление <> Перечисления.НаправленияЭДО.Внутренний
...
Процедура ПодготовитьПредставлениеДокумента(ЭлементСхемыРегламента)
	...
	ИнтерфейсДокументовЭДО.ДополнитьТабличныйДокументШтампамиПодписей(ПредставлениеДокумента, ЭлементСхемыРегламента.Сообщение);
...
Процедура ЗаполнитьПодписи(ЭлементСхемы)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(УстановленныеПодписи, Подписи);
	ИнтерфейсДокументовЭДО.ЗаполнитьПредставлениеСостоянияПодписей(Подписи);
			
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТекстСоспроводительнойЗаписки(ЭлементСхемы);
...
Процедура СформироватьПредупреждения()
	...
	Если Объект.СпособОбмена <> Перечисления.СпособыОбменаЭД.Внутренний Тогда
...
&НаКлиенте
Процедура ОткрытьПомошникСОшибкойЗапретаФормирования(РезультатПроверки)

	ПараметрыФормы = НастройкиОтправкиЭДОКлиент.НовыеПараметрыФормыНастроекОтправки();
	ПараметрыФормы.КлючНастроекОтправки = РезультатПроверки.КлючНастроекОтправки;
	...
	Описание.ТипДокумента = ЭлементСхемыИнформацияОтправителя(ЭтотОбъект).ТипДокумента;
	...
	ИнтерфейсДокументовЭДОКлиент.ПоказатьПроблемыПриОбработкеДокументов(Новый ОписаниеОповещения, ПараметрыОткрытия);
КонецПроцедуры
...
Процедура ПослеПроверкиПодписейДокумента(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбновитьДанныеЭлектронногоДокументаНаСервере();
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаПодписиИСтатусы;
...
&НаСервере
Процедура ДобавитьВложениеИзДвоичныхДанных(ДвоичныеДанные, НаименовниеФайла, Расширение)
	
	АдресВХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор);
	
	Объект.Комментарий = НаименовниеФайла;  
	
	Если НЕ ОтключитьТранслитерацию Тогда
		НаименовниеФайла = СтроковыеФункции.СтрокаЛатиницей(НаименовниеФайла);
	КонецЕсли;
	
	НаименовниеФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(НаименовниеФайла, "_");
	
	НаименовниеФайла = СтрШаблон("%1.%2", НаименовниеФайла, Расширение);
				
	ЭлементСхемы = ЭлементСхемыИнформацияОтправителя(ЭтотОбъект);	
	
	ЭлементСхемы.АдресФайла = АдресВХранилище;
	ЭлементСхемы.ИмяФайла = НаименовниеФайла;
	ЭлементСхемы.РасширениеФайла = Расширение;
...
Процедура ОбновитьОтображениеМаршрутаПодписания()
	...
	Элементы.МаршрутПодписания.Доступность = ЗначениеЗаполнено(Объект.Организация);
...
Функция ПроверитьЗаполнениеДокумента(ЭлементСхемыРегламента)
	...
	Если Не ЗначениеЗаполнено(Объект.МаршрутПодписания) И Не ЗначениеЗаполнено(Объект.СписокПодписантов) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ШаблонСообщенияОНезаполненности, "Маршрут подписания"),
			Объект.Ссылка, "МаршрутПодписанияДокумента", "Объект", Результат.Отказ);
	КонецЕсли;
...
Процедура ОткрытьНастройкиОтправки(Команда)
	...
	ПараметрыОткрытия = НастройкиЭДОКлиент.НовыеПараметрыТранстпортныхНастроек();
