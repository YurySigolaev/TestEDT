...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ЭлектронныйДокумент", Документооборот) Или
		 Не Параметры.Свойство("ВидДокумента", ВидДокумента) Тогда		
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура СоздатьДокументУчетаНаСервере(СпособОбработки, КонтекстОперации)
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД Или
			ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД Тогда
...
Процедура ПерепривязатьЭлектронныйДокумент(ВыбранноеЗначение, СпособОбработки)
	...
	Если Элементы.ГруппаНастройкиОтраженияВУчете.Видимость Тогда
		...
		КлючНастройки.Получаетль = Документооборот.Организация;	
