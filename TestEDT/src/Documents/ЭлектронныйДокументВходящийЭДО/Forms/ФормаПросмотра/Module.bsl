...
Процедура ОткрытьФайлВложения(ДляРедактирования = Ложь)
	...
	Если  ЗначениеЗаполнено(ЭлементСхемы.ПрисоединенныйФайл) Тогда
		ДанныеФайла = РаботаСФайламиКлиент.ДанныеФайла(ЭлементСхемы.ПрисоединенныйФайл, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ТребуетсяСопоставлениеНоменклатурыДляДокументаСОтключенымКонтролем(ДанныеДляОтраженияВУчете)
...
Процедура ОбработатьВыборСозданияДокументаУчета(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ТребуетсяСопоставлениеНоменклатурыДляДокументаСОтключенымКонтролем(ДанныеДляОтраженияВУчете()) Тогда
