...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_НаборКонстант"
			И (Источник = "ОтправлятьВходящиеДокументыНаУтверждение" Или
			   Источник = "ИспользоватьОтложеннуюОтправкуЭлектронныхДокументов") Тогда
...
Процедура БыстрыеОтборыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СтрНайти(Поле.Имя, "Кнопка") <> 0 Тогда
		...
		ИнтерфейсДокументовЭДОКлиент.ОткрытьНастройкиСпискаТекущиехДел(Оповещение, ЭтотОбъект, Список.КомпоновщикНастроек)
...
Процедура ОтправитьПриглашения(Команда)
	...
	ИсходящиеПрилашения = Новый Массив;
	...
	Для Каждого ДанныеПриглашения Из ДанныеПриглашений Цикл	
		...
		ИсходящиеПрилашения.Добавить(ИсходящееПриглашение);
	КонецЦикла;
	
	ОбработчикОповещения = Новый ОписаниеОповещения("ОтправкаПриглашенийОповещениеОЗавершении", ЭтотОбъект);
		
	СинхронизацияЭДОКлиент.ОтправитьПриглашения(ИсходящиеПрилашения, ЭтотОБъект, ОбработчикОповещения);
...
Функция ПоляРаздела(Знач Раздел)
	...
	ПоляТаблицыВходящие = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО, НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете, ЭлектронныйДокумент";
	ПоляТаблицыИсходящие = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, Подписи, СостояниеЭДО, НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете, ЭлектронныйДокумент";
	...
	ПоляТаблицыОзнакомиться = "ВидДокумента, Дата, Номер, СостояниеЭДО, Организация, Контрагент, СуммаДокумента, ЭлектронныйДокумент";
	ПоляТаблицыОтправить = "ВидДокумента, Дата, Номер, Организация, Контрагент, СуммаДокумента, ЭлектронныйДокумент";
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, Знач НастройкиОтображения,
	...
	ИначеЕсли Раздел = "Подписать" Тогда
		...
		Значение = ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяПодписание");
...
Процедура УстановитьБыстрыеОтборыПоРазделу(Знач Раздел, Знач БыстрыеОтборы,
	...
	Для каждого СтрокаОтбора Из БыстрыеОтборы Цикл
		...
		ИначеЕсли Имя = "ДополнительнаяИнформация" Тогда
			...
			ГруппаИЛИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаБыстрыйОтбор, НСтр("ru = 'Дополнительная информация'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИЛИ);

			Использование = ЗначениеЗаполнено(Значение);

			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИЛИ, "ДополнительнаяИнформация", ВидСравненияКомпоновкиДанных.Содержит, Значение, , Использование, Режим);
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИЛИ, "ПричинаОтклонения", ВидСравненияКомпоновкиДанных.Содержит, Значение, , Использование, Режим);
...
Процедура УстановитьУсловноеОформлениеПоИспользованиюПакетногоОтображения()
	...
	Если ПакетноеОтображение Тогда	 
		...
		ГруппаОтбораПакетСсылка = ГруппаОтбора.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
		ГруппаОтбораПакетСсылка.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
		
		ГруппаОтбораПакета = ГруппаОтбораПакетСсылка.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
		ГруппаОтбораПакета.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	
		ОтборЭлемента = ГруппаОтбораПакета.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		...
		ГруппаОтбораСсылка = ГруппаОтбораПакетСсылка.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
		ГруппаОтбораСсылка.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
		
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИсходящиеЭД.ЭлектронныйДокумент");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("ТекущийЭлектронныйДокумент");
		
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИсходяшиеЭД.КлючСортировкиПакета");
		...
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВходящиеЭД.ЭлектронныйДокумент");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("ТекущийЭлектронныйДокумент");
		...
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОшибкиЭД.ЭлектронныйДокумент");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("ТекущийЭлектронныйДокумент");
		...
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОтправитьЭД.ЭлектронныйДокумент");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("ТекущийЭлектронныйДокумент");
		...
		ОтборЭлемента = ГруппаОтбораСсылка.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОзнакомитьсяЭД.ЭлектронныйДокумент");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("ТекущийЭлектронныйДокумент");
...
Процедура РассчитатьДополнительныеПоляПакетов(Строки)
	...
	Для Каждого ТекущаяСтрока Из Строки Цикл
		...
		Если ТекущиеИдентификаторы <> Неопределено Тогда
			...
			Если ДанныеСтроки.НомерСлужебный = ТекущиеИдентификаторы[ТекущиеИдентификаторы.Количество()-1].НомерСлужебный И 
				ДанныеСтроки.КоличествоДокументовВПакете > КоличествоОтображаемыхДокументов Тогда
				
				КоличествоСкрытыхДокументов = ДанныеСтроки.КоличествоДокументовВПакете - КоличествоОтображаемыхДокументов;
				ШаблонСтроки = НСтр("ru = ';%1 документ;;%1 документа;%1 документов;%1 документов'");
				ДанныеСтроки.ПредставлениеДокументовВнеОтбора = НСтр("ru = 'и еще '") + СтрокаСЧислом(ШаблонСтроки,
					КоличествоСкрытыхДокументов, ВидЧисловогоЗначения.Количественное);
...
Процедура УстановитьВариантОтображенияДокументовСписка(ИмяСписка)
	...
	Если ПакетноеОтображение Тогда	
		...
		БазовыеНастрокиКомпановки = Новый КомпоновщикНастроекКомпоновкиДанных;
		БазовыеНастрокиКомпановки.ЗагрузитьНастройки(ЭтотОбъект[ИмяСписка].КомпоновщикНастроек.Настройки);
		БазовыеНастрокиКомпановки.Настройки.Порядок.Элементы.Очистить();
		
		ЭтотОбъект[ИмяСписка].КомпоновщикНастроек.ЗагрузитьПользовательскиеНастройки(БазовыеНастрокиКомпановки.ПользовательскиеНастройки);
...
Процедура ОбновитьЗаголовкиЭлементов()
	...
	Если НастройкиЭДО.ИспользуютсяВнутренниеДокументы() Тогда
		ЗаголовокКолонкиКонтрагентИсходяшихЭД = НСтр("ru = 'Контрагент / Организация'");
	Иначе
		ЗаголовокКолонкиКонтрагентИсходяшихЭД = НСтр("ru = 'Контрагент'");
	КонецЕсли;
	
	Элементы.ИсходящиеЭДКонтрагент.Заголовок = ЗаголовокКолонкиКонтрагентИсходяшихЭД;
	Элементы.СформироватьЭДКонтрагент.Заголовок = ЗаголовокКолонкиКонтрагентИсходяшихЭД;
...
Процедура ОтправитьОтветНаПриглашения(Действие)
	...
	ОчиститьСообщения();
	ДанныеПришлашений = ДанныеВыделенныхСтрокРаздела(ТекущийРаздел, "КлючПриглашения, ИдентификаторОрганизации, ИдентификаторКонтрагента, СпособОбменаЭД, Контрагент, НовыйКонтрагент");
	
	ВходящиеПриглашения = Новый Массив;

	Для Каждого ДанныеПришлашения Из ДанныеПришлашений Цикл
		ВходящееПриглашение = СинхронизацияЭДОКлиент.НовоеВходящееПриглашение();
		ЗаполнитьЗначенияСвойств(ВходящееПриглашение, ДанныеПришлашения);
		ВходящееПриглашение.СпособОбмена = ДанныеПришлашения.СпособОбменаЭД;
...
Процедура ОткрытьЭлектронныйДокументДляПросмотра(ДанныеСтроки)
	...
	// TODO Burachkov проверить возможность изменения ключа замера
