...
Процедура ПодписатьВсе(Команда)
	...
	ПодписатьДокументы(МассивДокументовНаПодпись);
...
Процедура ПослеПолученияОтпечатковПерезаполнитьТаблицы(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Результат.Клиент <> Неопределено И Результат.Клиент.Ошибка Или
			Результат.Сервер <> Неопределено И Результат.Сервер.Ошибка Или
			Результат.Облако <> Неопределено И Результат.Облако.Ошибка Тогда
		Закрыть();
		Возврат;
	КонецЕсли;
