...
Функция СформироватьНаборВажныхПолей(Знач ОбъектСсылка, Версия = Неопределено,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ОсобыеДанныеПодписания")
	...
	Для Каждого ИмяПоля Из МассивИмен Цикл
		...
		Иначе  // табличные части отдельно обработать
			...
			Для Каждого Строка Из ОбъектСсылка[ИмяТабличнойЧасти] Цикл
				...
				Если ИмяТабличнойЧасти = "Стороны" Тогда
					
					ДанныеПодписанияСтроки = ДанныеПодписания.Получить(НомерСтроки);
					
					Если ДанныеПодписанияСтроки <> Неопределено Тогда
						Если ИмяКолонки = "Подписал" Тогда
							ЗначениеПоля = ДанныеПодписанияСтроки.Подписал;
						ИначеЕсли ИмяКолонки = "Подписан" Тогда
							ЗначениеПоля = ДанныеПодписанияСтроки.Подписан;
						ИначеЕсли ИмяКолонки = "ДатаПодписи" Тогда
							ЗначениеПоля = ДанныеПодписанияСтроки.ДатаПодписи;
						КонецЕсли;
					КонецЕсли;
	...
	Попытка
		...
		Для Каждого Данные Из ПодписанныеДанные Цикл
			...
			Иначе
				...
				ИначеЕсли ТипЗнч(ПодписываемыйОбъект) = Тип("СправочникСсылка.СообщениеЭДОПрисоединенныеФайлы")
					Или ТипЗнч(ПодписываемыйОбъект) = Тип("СправочникОбъект.СообщениеЭДОПрисоединенныеФайлы") Тогда
