...
Функция СформироватьДанныеВнутреннегоДокумента(ОбъектУчета, Настройки) Экспорт
	...
	РезультатФормирования.Содержание.НомерДокумента = ДанныеДокумента.НомерДокумнета;
...
Функция ВнутреннийДокументооборотАктуален(ОбъектУчета, ВидЭлектронногоДокумента, ДвоичныеДанныеАрхива) Экспорт
	...
	СуществующийТабличныйДокумент.Записать(Поток);
	ДанныеСуществующегоПредставления = Поток.ЗакрытьИПолучитьДвоичныеДанные();
	
	Возврат ДанныеТекущегоПредставления = ДанныеСуществующегоПредставления;
...
Функция СформироватьДокумент(ОбъектУчета, ВидДокумента)
	...
	Описание.Вставить("НомерДокумнета", НомерДокумента);
...
// Иницализирует содержание документа.
//
// Возвращаемое значение:
