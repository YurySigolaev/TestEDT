...
// Возвращает вид ошибки, при ошибках обработки документов.
// 
// Возвращаемое значение:
// 	См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиПроблемПриОбработкеДокументов() Экспорт
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "ПроблемыПриОбработкеДокументов";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Возникли проблемы при обработке документов'");;
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""Запустить"">Запустить</a> обработку ошибок'");
	ВидОшибки.ОбработчикиНажатия.Вставить("Запустить", ОбработчикОткрытияПроблемПриФормировании());
...
// 	* Вид - Строка - идентификатор вида ошибки.
// 	* Данные - Структура - данные ошибки.
//
Функция НоваяСтруктураОшибкиОбработкиДокументов() Экспорт
	
	СтруктураОшибки = Новый Структура;
	СтруктураОшибки.Вставить("Вид");
	СтруктураОшибки.Вставить("Данные");
	
	Возврат СтруктураОшибки
	
КонецФункции

Функция НовыеПараметрыОшибокЗаполнения(Ошибки, ИмяРаздела = "ОсновныеПоля") Экспорт
	
	Результат = Новый Структура;
	Если ЗначениеЗаполнено(Ошибки) Тогда
		Результат.Вставить(ИмяРаздела, Ошибки);
	КонецЕсли;
...
Функция НовыеПараметрыПомощникаУстраненияОшибокОбработкиЭД(СсылкаНаОбъект, ОшибкиЗаполнения, ЭтоПредупреждение = Ложь) Экспорт
	
	ДанныеДляОбработкиОшибок = Новый Структура;
	ДанныеДляОбработкиОшибок.Вставить("Документ", СсылкаНаОбъект);
	
	Результат = Новый Структура;
	Результат.Вставить("ОшибкиПриФормированииДокумента", Новый Структура);
	
	Результат.ОшибкиПриФормированииДокумента.Вставить("ДанныеДляОбработкиОшибок", ДанныеДляОбработкиОшибок);
	Результат.ОшибкиПриФормированииДокумента.Вставить("ОшибкиЗаполнения"        , ОшибкиЗаполнения);
	Результат.ОшибкиПриФормированииДокумента.Вставить("ЭтоПредупреждение"       , ЭтоПредупреждение);
...
Функция ГраницыВыводаДокументовПакета(СоставПакета, ТекущийДокумент, КоличествоОтображаемыхДокументовПакета) Экспорт
	...
	ДополнительноВыдодимыхСправа = КоличествоОтображаемыхДокументов - ПозицияДокументаНаПанели;	
	ДополнительноВыдодимыхСлева = ПозицияДокументаНаПанели - 1;

	Результат = Новый Структура;
	Результат.Вставить("ИндексНачалоВыводаЭлементов", ИндексДокумента - ДополнительноВыдодимыхСлева);
	Результат.Вставить("ИндексКонцаВыводаЭлементов", ИндексДокумента + ДополнительноВыдодимыхСправа);
...
// 	* Блокирующее - Булево - признак невозможности дольнейших действий без устранения проблемы.
...
Функция ОбработчикОткрытияПроблемПриФормировании()
	Возврат "ИнтерфейсДокументовЭДОКлиент.ПоказатьПроблемыПриФормированииДокументов";
КонецФункции
...
// 	* ФормаВладелец - ФормаКлиентскогоПриложения - форма.
//
