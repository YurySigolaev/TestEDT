...
//   * Формат - Строка - формат документа (см. ПоддерживаемыеФорматы).
//   * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО - вид докумнета.
//   * ВыгружатьДополнительныеСведения - Булево - признак выгрузки дополнительных сведений.
//   * ОтключитьРассчетДополнительныхПолей - Булево - принак отключения расчсчета дополнительных полей.
...
// Инициализирует настройки формирования электронного документа по настройке ЭДО.
//
// Возвращаемое значение:
//  Структура - параметры формирования:
//   * Формат - Строка - формат документа (см. ПоддерживаемыеФорматы).
//   * ТипДокумента - СправочникСсылка.ВидыДокументовЭДО - вид докумнета.
//   * Отправитель - ОпределяемыйТип.Организация - отправитель документа.
//   * ВыгружатьДополнительныеСведения - Булево - признак выгрузки дополнительных сведений.
//   * ЗаполнениеКодаТовара - Строка - значение из списка, заданного в 
//                                     ПриОпределенииВариантовЗаполненияПолейЭлектронныхДокументов.
//
Функция НовыеНастройкиФормированияДокументаПоНастройкеЭДО() Экспорт
...
//
Функция НовыеНастройкиФормированияПрикладногоДокумента() Экспорт
...
//      *** Тип - Строка - "Оператор" или "Абонент" в зависимости от типа Пполучателя.
...
// Инициализирует данные для формирования по насртроке ЭДО.
//
// Возвращаемое значение:
//  Структура - данные для заполнения:
//   * ВерсияПрограммы - Строка - версия программы, с помощью которой сформирован файл. По умолчанию "1С:Предприятие 8".
...
//     *** Идентификатор - Строка - идентификатор отправителя.
//
Функция НовыеДанныеДляФормированияПоНастройкеЭДО() Экспорт
	...
	Данные.Вставить("УникальныйИдентификатор");
	Данные.Вставить("Участники", Новый Структура);
	Данные.Вставить("ПараметрыФормирования", Новый Структура);
	Данные.Участники.Вставить("ИдентификаторОтправителя", "");
	Данные.Участники.Вставить("ИдентификаторПолучателя", "");
	Данные.Участники.Вставить("Оператор", Новый Структура);
	Данные.Участники.Оператор.Вставить("Наименование", "");
...
// 	* ПубличнаяСсылкаQRКода - Строка -  текст публичной сылки QR кода.
//
...
Функция ДополнитьДокументДаннымиДополнительныхПолей(ОбъектУчета, ПараметрыПолучения, ДеревоДанных, КэшДополнительныхДанных) Экспорт
...
Процедура ПолучитьЗапросКонструктораДополнительныхПолейТаблицы(ТекстЗапроса, ВычислениеЗначений = Ложь) Экспорт
...
Функция ВычислитьЗначенияДополнительныхПолейШапки(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	Для Каждого НастройкаПоФормуле Из МассивНастроекПоФормуле Цикл
		...
		Попытка 
			ЗначениеФормулы = ОбщегоНазначенияБЭД.РезультатВычисленияФормулы(Формула, ДанныеЗначенийОперандов); //todo: Получать версию механики расчета из настроек
...
//		** ЗаполнениеДанных - Массив - ошибки заполения данных. 
//		** ЗначенияДополнительныхПолей - Массив - ошибки заполения дополнительных данных.
...
//
Функция ДанныеПрикладногоЭлектронногоДокумента(ОбъектУчета, ПараметрыФормирования) Экспорт
...
#Область ФортированиеСодержания

Функция ТипРегламентаПоЗначениюСервиса(Значение) Экспорт
...
КонецФункции
// Водвращает данные электронного документа.
//
// Параметры:
...
//	Если ЗначениеЗаполнено(ПараметрыПолученияДанных.ФайлКартинок.ДвоичныеДанные) Тогда
//		ДеревоКартинок = ДеревоКартинок(ФайлКартинок);
//		ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДеревоКартинок", ДеревоКартинок);
//	КонецЕсли;	
...
Функция ОпределитьПараметрыФайлаПрикладногоЭлектронногоДокумента(ПараметрыПолученияДанных, РасширениеФайла) Экспорт
...
Функция ВидыПередаточногоДокументаУПД(ДанныеУПД) Экспорт
	
	Товары = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУПД, "Документ.ТаблСчФакт.СведТов",, Истина);
	Если Товары = Неопределено Тогда
		Товары = Новый Массив;
	КонецЕсли;
	...
	Если ЕстьПризнак Тогда
		Если ТолькоУслуги Тогда
			ВидыПередаточногоДокумента.Добавить(ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот));
		ИначеЕсли ТолькоПрава Тогда
			ВидыПередаточногоДокумента.Добавить(ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав));
		Иначе
			ВидыПередаточногоДокумента.Добавить(ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная));
		КонецЕсли;
	Иначе
		ВидыПередаточногоДокумента.Добавить(ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная));
		ВидыПередаточногоДокумента.Добавить(ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот));
...
Функция ТекстУточнения(ОписаниеФайла) Экспорт
	...
	ИначеЕсли СтрНайти(ОписаниеФайла.ИмяФайла, "DP_PRANNUL") > 0 
		И Тип(Данные.Документ.СвПредАн.ТекстПредАн) = Тип("Строка") Тогда
