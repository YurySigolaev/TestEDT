...
Функция БазаСозданаЧерезOnboarding() Экспорт
	
	ПодключенПоOnboarding = Ложь;
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ЕстьПодсистемаКлиентЭДО = ОбщегоНазначения.ПодсистемаСуществует("КлиентЭДО");
		Если ЕстьПодсистемаКлиентЭДО Тогда
			МодульКлиентЭДО = ОбщегоНазначения.ОбщийМодуль("КлиентЭДО");
			НастройкиСтартовогоПомощникаКЭДО = МодульКлиентЭДО.ПолучитьНастройкиСтартовогоПомощникаКЭДО();
