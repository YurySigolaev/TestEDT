...
Процедура ОткрытьОшибкиСертификата(Форма, Сертификат, РезультатПроверки) Экспорт
	...
	СерверныеПараметры.Вставить("ЗаголовокФормы", НСтр("ru = 'Результаты проверки сертификата'"));
	...
	СерверныеПараметры.Вставить("РезультатПроверки", РезультатПроверки);
	СерверныеПараметры.Вставить("ОбъединятьРезультаты", "ОбъединятьПоИли");
	
	ЭлектроннаяПодписьСлужебныйКлиент.ФормаПередачаПараметров().ОткрытьНовуюФорму("ПроверкаСертификата",
		СерверныеПараметры, Новый Структура, , Форма);
