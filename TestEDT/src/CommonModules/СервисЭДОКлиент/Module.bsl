...
// * Операция - Строка -
Функция ПодготовитьДанныеНаПодпись(ДанныеПакета) Экспорт
...
Процедура ПолучитьДанныеЗашифрованногоМаркераПродолжить(Контекст, ПредложитьПодключитьИнтернетПоддержку = Истина)
	...
	Если ДанныеСертификата <> Неопределено Тогда
		ДанныеСертификата.Свойство("МаркерЗашифрованный", МаркерЗашифрованный);
	КонецЕсли;
	...
	Если МаркерЗашифрованный = Неопределено Тогда
		...
		ТекстОшибки = СтрШаблон(ТекстОшибкиШаблон, Контекст.ВыбранныйСертификат,
...
Процедура ОбработатьПредставлениеДанныхДляОператораЭДО(РезультатВыполнения, Контекст) Экспорт
	...
	ОткрытьДанныеДляРегистрацииУОператораЭлектронногоДокументооборота(Контекст.РеквизитыПакета,
		Контекст.ДвоичныеДанныеДляОператораЭДО);
