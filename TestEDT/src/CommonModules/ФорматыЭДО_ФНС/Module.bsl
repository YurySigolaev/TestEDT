...
Функция ПроверитьЗаполнениеДанных_УПД2019_ИнформацияПродавца(Данные)
	...
	Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
		...
		Если ЗначениеЗаполнено(ЕдиницаИзмеренияКод) Тогда
			...
			Если (ФункцияУПД = СЧФДОП() ИЛИ ФункцияУПД = ДОП()) И Не ЗначениеЗаполнено(Количество) Тогда
		...
		Если ФункцияУПД = СЧФ() ИЛИ ФункцияУПД = СЧФДОП() Тогда
			...
			Если Не (ЗначениеЗаполнено(ВидСчетаФактуры) И ВидСчетаФактуры = "Авансовый") И Не ЗначениеЗаполнено(СтоимостьБезНДС) Тогда
				ТекстОшибки = ДеревоЭлектронногоДокументаБЭД.ТекстОшибкиНеверноЗаполненногоЗначенияДерева();
				ДеревоЭлектронногоДокументаБЭД.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтоимостьБезНДССтрокаДерева, ТекстОшибки,, НомерСтроки);
			КонецЕсли;
		КонецЕсли;
	...
	Если ФункцияУПД = СЧФ() ИЛИ ФункцияУПД = СЧФДОП() Тогда
		...
		Если Не (ЗначениеЗаполнено(ВидСчетаФактуры) И ВидСчетаФактуры = "Авансовый") И Не ЗначениеЗаполнено(СтоимостьБезНДСВсего) Тогда
			ТекстОшибки = ДеревоЭлектронногоДокументаБЭД.ТекстОшибкиНеверноЗаполненногоЗначенияДерева();
			ДеревоЭлектронногоДокументаБЭД.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтоимостьБезНДСВсегоСтрокаДерева, ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
	...
	Если ФункцияУПД <> ДОП() И Не ЗначениеЗаполнено(СсылкаСчетаФактуры) Тогда
...
Функция ПроверитьЗаполнениеДанных_УКД2020_ИнформацияПродавца(Данные)
	...
	Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
		...
		СтавкаБезНДС = ЭтоСтавкаБезНДС(НалоговаяСтавка);
		...
		ПараметрыПроверки.СтавкаБезНДС = СтавкаБезНДС;
...
Процедура ПроверитьСтоимостьУвеличенияУменьшенияТовараУКД_2020(ПараметрыПроверки, СтоимостьСНалогом, Ошибки)
	...
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьУвеличение);
	СтоимостьУвеличение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
	
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьУменьшение);
	СтоимостьУменьшение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
...
Процедура ПроверитьСтоимостьТовараПослеИзмененияУКД_2020(ПараметрыПроверки, СтоимостьСНалогом, Ошибки)
	...
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьТовара);
	СтоимостьТовара = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
	
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьТовараДо);
	СтоимостьТовараДо = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
...
Процедура ПроверитьСуммуНалогаУКД_2020(ПараметрыПроверки, Ошибки) 
	...
	Если Не Количество = 0
		...
		И Не ПараметрыПроверки.СтавкаБезНДС Тогда
			
		ПолныйПуть = НСтр("ru = 'СведенияОТоварах.НомерСтроки.СуммаНалога'");
		СуммаНалога = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
		...
		ПолныйПуть = НСтр("ru = 'СведенияОТоварах.НомерСтроки.СуммаНалогаУвеличение'");
		СуммаНалогаУвеличение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
		
		ПолныйПуть = НСтр("ru = 'СведенияОТоварах.НомерСтроки.СуммаНалогаУменьшение'");
		СуммаНалогаУменьшение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ПараметрыПроверки.ДеревоДанных, ПолныйПуть);
		...
		Если ТипЗнч(Реквизит) = Тип("Число") И Реквизит > 0 Тогда
			...
			Если ТипЗнч(СтрокаТаблицы.Значение) = Тип("Число") И СтрокаТаблицы.Значение > 0 Тогда
				Для Каждого Строка Из СтрокаТаблицы.Строки Цикл
					ПолныйПуть = СтрШаблон(НСтр("ru ='%1.НомерСтроки.ДокументНаименование'"), Элемент.Ключ);
					ДокументНаименование = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Строка, ПолныйПуть);
					
					ПолныйПуть = СтрШаблон(НСтр("ru ='%1.НомерСтроки.ДокументДата'"), Элемент.Ключ);
					...
					Если ЗначениеЗаполнено(ДокументНаименование) И Не ЗначениеЗаполнено(ДокументДата) Тогда
						...
						ДеревоЭлектронногоДокументаБЭД.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, Строка, ТекстОшибки);
...
Функция ЗаполнитьДокумент(Знач Формат, Знач Данные)
	...
	ИначеЕсли Формат = Форматы.БизнесСеть.КоммерческоеПредложение Тогда
		...
		Документ = ЗаполнитьДокумент_КоммерческоеПредложение(Данные);
		
	ИначеЕсли Формат = Форматы.БизнесСеть.ЗапросКоммерческогоПредложения Тогда	
		
		Документ = ЗаполнитьДокумент_ЗапросКоммерческогоПредложения(Данные);
...
Функция ЗаполнитьДокумент_УПД2019_ИнформацияПродавца(Знач Данные)
	...
	Если ЗначениеЗаполнено(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "ДополнительныеСведенияОбУчастниках")) Тогда
		...
		Если Документ.Функция = "СЧФ" Тогда
	...
	Иначе
		...
		Результат.ДанныеОсновногоФайла.ДвоичныеДанные = УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИмен);
...
Функция ЗаполнитьДокумент_ПередачаТоваров_ИнформацияПродавца(Знач Данные)
	...
	ГрузОтпр = ПолучитьОбъектТипаCML("УчастникТип", ПространствоИмен);
	ЗаполнитьСведенияОбУчастникеПередачаТоваров(ГрузОтпр, ДанныеГрузоотправителя, Ошибки, ПространствоИмен, "Грузоотправитель");
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СодФХЖ1, "ГрузОтпр", ГрузОтпр, Ложь, Ошибки);
	
	ДанныеГрузполучателя = ДеревоДанных.Строки.Найти("Грузополучатель", "ПолныйПуть");
	
	ГрузПолуч = ПолучитьОбъектТипаCML("УчастникТип", ПространствоИмен);
	ЗаполнитьСведенияОбУчастникеПередачаТоваров(ГрузПолуч, ДанныеГрузполучателя, Ошибки, ПространствоИмен, "Грузополучатель");
	...
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СодФХЖ1, "ВидОперации", XMLСтрока(ВидОперации), , Ошибки);
	...
	Если ЗначениеЗаполнено(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "СведенияОЛицеПередавшемТовары")) Тогда
			...
			ИначеЕсли ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "СведенияОЛицеПередавшемТовары") = "ИноеЛицо" Тогда
				...
				ИначеЕсли ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "СведенияОЛицеПередавшемТовары.ИноеЛицо") = "ФЛ" Тогда
					...
					РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ИнЛицо, "ФЛПер", ФЛ, , Ошибки);
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СодФХЖ3, "СвЛицПер", СвЛицОтпГруз, , Ошибки);
...
Функция ЗаполнитьДокумент_АктОРасхождениях_ИнформацияПокупателя(Знач Данные)
	...
	Для Каждого СтрокаРезультата Из РезультатыПриемки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаРезультата.ПоДокументу.Количество) Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ПоДокум, "СтавНДСПрин", СтавкаНДСXDTO(СтавкаНДС), , Ошибки);
		...
		Если ЗначениеЗаполнено(СтрокаРезультата.ПоФакту.Количество) Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ПоФакту, "СтавНДСПрин", СтавкаНДСXDTO(СтавкаНДС), , Ошибки);
...
Функция ЗаполнитьДокумент_УКД_ИнформацияПокупателя(Знач Данные)
	...
	ДатаИнформацииПродавца = ДатаФНС(ИнформацияПродавца.Документ.ДатаИнфПр);
...
Функция ЗаполнитьДокумент_УКД2020_ИнформацияПродавца(Знач Данные)
	...
	СчФ = ПолучитьОбъектТипаCML("Файл.Документ.СвКСчФ.СчФ", ПространствоИмен);
	НомерДок = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "НомерИсходногоДокумента");
	ДатаДок = ДатаФНС(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных,
		"ДатаИсходногоДокумента"));
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СчФ, "НомерСчФ", НомерДок, Истина, Ошибки);
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СчФ, "ДатаСчФ", ДатаДок, Истина, Ошибки);

	НомерДок = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных,
		"НомерИсправленияИсходногоДокумента");
	ДатаДок = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных,
		"ДатаИсправленияИсходногоДокумента");
	...
	Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
		...
		СуммаАкцизаУвеличение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
			"СведенияОТоварах.НомерСтроки.СуммаАкцизаУвеличение");
		СуммаАкцизаУменьшение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
			"СведенияОТоварах.НомерСтроки.СуммаАкцизаУменьшение");

		СуммуУвеличенияУменьшения = ПолучитьИмяИЗначениеСуммыРазнИзДереваУКД_2020(СуммаАкцизаУвеличение,
			СуммаАкцизаУменьшение);
		Если ЗначениеЗаполнено(СуммуУвеличенияУменьшения.Значение) Тогда
			РазнСумНалТип = ПолучитьОбъектТипаCML("РазнСумНалТип", ПространствоИмен);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(РазнСумНалТип, СуммуУвеличенияУменьшения.Имя, СуммуУвеличенияУменьшения.Значение, Истина,
				Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОТоваре, "АкцизРазн", РазнСумНалТип, Истина, Ошибки);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Реквизит) Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "СумНДС", Реквизит, Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОТоваре, "СумНалДо", СумНДСТип, Истина, Ошибки);
		ИначеЕсли ВРег(СтавкаНДСДо) = ВРег("без НДС") Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "БезНДС", "без НДС", Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОТоваре, "СумНалДо", СумНДСТип, Истина, Ошибки);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Реквизит) Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "СумНДС", Реквизит, Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОТоваре, "СумНалПосле", СумНДСТип, Истина, Ошибки);
		ИначеЕсли ВРег(СтавкаНДСПосле) = ВРег("без НДС") Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "БезНДС", "без НДС", Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОТоваре, "СумНалПосле", СумНДСТип, Истина, Ошибки);
		КонецЕсли;
		...
		СуммаНалогаУвеличение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
			"СведенияОТоварах.НомерСтроки.СуммаНалогаУвеличение");
		СуммаНалогаУменьшение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
			"СведенияОТоварах.НомерСтроки.СуммаНалогаУменьшение");

		СуммуУвеличенияУменьшения = ПолучитьИмяИЗначениеСуммыРазнИзДереваУКД_2020(СуммаНалогаУвеличение,
		...
		КонецЕсли;
			...
			// Стоимость без налога
		ЗаполнитьСтоимостьТовараУКД_2020(СведенияОТоваре, Товар, ПространствоИмен, Ложь, Ошибки);
			
			// Стоимость с налогом
		ЗаполнитьСтоимостьТовараУКД_2020(СведенияОТоваре, Товар, ПространствоИмен, Истина, Ошибки);
		...
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДопСведТов, "НаимЕдИзмДо", Реквизит, , Ошибки);
		Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
			"СведенияОТоварах.НомерСтроки.НаименованиеЕдиницыИзмеренияПослеКорректировки");
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДопСведТов, "НаимЕдИзмПосле", Реквизит, , Ошибки);
		Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар,
		...
		Для Каждого СтрокаПрослеживаемости Из СведенияОПрослеживаемости Цикл
			...
			ИзмененияВКоличестве = ПолучитьОбъектТипаCML("Файл.Документ.ТаблКСчФ.СведТов.СведПрослеж.ИзмКолБдПТ",
				ПространствоИмен);
			Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
					Товар,
				"СведенияОТоварах.НомерСтроки.СведенияОПрослеживаемости.НомерСтроки.КоличествоТовараДоИзменения");
			ИзмененияВКоличестве.КолТовЕдПТДо = Реквизит;
			Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
					Товар,
				"СведенияОТоварах.НомерСтроки.СведенияОПрослеживаемости.НомерСтроки.КоличествоТовараПослеИзменения");
			ИзмененияВКоличестве.КолТовЕдПТПосле = Реквизит;
			Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
					Товар,
				"СведенияОТоварах.НомерСтроки.СведенияОПрослеживаемости.НомерСтроки.КоличествоТовараУвеличение");
			Если ЗначениеЗаполнено(Реквизит) Тогда
				ИзмененияВКоличестве.КолТовЕдПТУвел = Реквизит;
			Иначе
				Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
						Товар,
					"СведенияОТоварах.НомерСтроки.СведенияОПрослеживаемости.НомерСтроки.КоличествоТовараУменьшение");
				ИзмененияВКоличестве.КолТовЕдПТУм = Реквизит;
			КонецЕсли;
			СведПрослеж.ИзмКолБдПТ = ИзмененияВКоличестве;
	...
	Если ЗначениеЗаполнено(Реквизит) Тогда
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "СумНДС", Реквизит, Истина, Ошибки);
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ВсегоУвел, "СумНал", СумНДСТип, Истина, Ошибки);
	ИначеЕсли ВсеСтрокиБезНДСДо И ВсеСтрокиБезНДСПосле Тогда
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СумНДСТип, "БезНДС", "без НДС", Истина, Ошибки);
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ВсегоУвел, "СумНал", СумНДСТип, Истина, Ошибки);
	КонецЕсли;
...
Функция ОбъектXDTO_УПД2019_ИнформацияПродавца(Знач ДанныеXML)
	...
	ПространствоИмен = ПространствоИмен_УПД2019_ИнформацияПродавца();
	Данные = ДобавитьПространствоИмен(ДанныеXML, ПространствоИмен);
	Объект = ОбъектXDTOДвоичныхДанных(Данные, ПространствоИмен, "Файл");
...
Функция ИдентификаторФайла_АктОРасхождениях_ИнформацияПокупателя(Знач Данные, Знач ДатаФормирования)
	...
	ИдентификаторФайла = СтрШаблон("DP_PRIRASXPRIN_%1_%2_%3_%4_%5",
		КонтрольДвиженияТовара,
		Участники.ИдентификаторПолучателя,
		Участники.ИдентификаторОтправителя,
		Формат(ДатаФормирования, "ДФ=yyyyMMdd"),
...
Процедура ЗаполнитьСведенияОбУчастникеПередачаТоваров(УчастникXDTO, СтрокаДереваДанных, Ошибки, ПространствоИмен, ВидУчастника)
	...
	ИначеЕсли ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(СтрокаДереваДанных, ВидУчастника + ".ТипУчастника") = "ФЛ" Тогда
		СвФЛ = ПолучитьОбъектТипаCML("УчастникТип.ИдСв.СвФЛУчастФХЖ", ПространствоИмен);
...
Процедура ЗаполнитьСтоимостьТовараУКД_2020(СведенияОТоваре, Товар, ПространствоИменСхемы, ЭтоСтоимостьСНалогом, Ошибки) 
	...
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьДо);
	СтоимостьДо = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, ПолныйПуть);
	Если ЗначениеЗаполнено(СтоимостьДо) Тогда
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СтоимТип, "СтоимДоИзм", СтоимостьДо, Истина, Ошибки);
		ЕстьДанные = Истина;
	КонецЕсли;
	
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьПосле);
	СтоимостьПосле = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, ПолныйПуть);
	Если ЗначениеЗаполнено(СтоимостьПосле) Тогда
	...
	Если ТипЗнч(СтоимостьУвеличение) = Тип("Число") Тогда
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СтоимТип, "СтоимУвел", СтоимостьУвеличение, , Ошибки);
		ЕстьДанные = Истина;
	КонецЕсли;
	
	ПолныйПуть = СтрШаблон(НСтр("ru = 'СведенияОТоварах.НомерСтроки.%1'"), ИмяСтоимостьУменьшение);
	СтоимостьУменьшение = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, ПолныйПуть);
	Если ТипЗнч(СтоимостьУменьшение) = Тип("Число") Тогда
...
Процедура ПрочитатьИнформациюПродавцаXDTO_5_01(ЭД, ДеревоРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭД, "Документ.СвСчФакт.ИнфПолФХЖ1") <> Неопределено Тогда
		...
		Если ТекстИнф <> Неопределено Тогда
			...
			Если НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Входящий Тогда
				...
				Для Каждого СтрокаИнформации Из ТекстИнф Цикл
					Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СтрокаИнформации, "Идентиф") = "ДанныеКомиссионера" Тогда
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭД, "Документ.ТаблСчФакт") <> Неопределено Тогда
		...
		Если СведТов <> Неопределено Тогда
			Для Каждого Товар Из СведТов Цикл
				...
				СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
					ИдентификаторТовара, НоваяСтрока.ТоварНаименование, "", НоваяСтрока.ЕдиницаИзмеренияНаименование);
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭД, "Документ.СвПродПер") <> Неопределено Тогда
		...
		ВидОперации = ПеречислениеИзЗначенияXML(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭД, "Документ.СвПродПер.СвПер.ВидОпер"), "ВидыОперацийЭД");
			...
			Если НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Входящий Тогда
				...
				Для Каждого СтрокаИнформации Из ТекстИнф Цикл
					Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СтрокаИнформации, "Идентиф") = "ДанныеКомиссионера" Тогда
						...
						СведенияОКомиссионере = ФабрикаXDTO.ПрочитатьXML(ОбъектXML);
						ОбъектXML.Закрыть();
						ЗаполнитьДанныеУчастникаУПД_2019(ДеревоДанных, СведенияОКомиссионере, "СведенияОКомиссионере");
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭД, "Документ.ТаблСчФакт") <> Неопределено Тогда
		...
		Если СведТов <> Неопределено Тогда
			Для Каждого Товар Из СведТов Цикл
				...
				Если ДопСведТов <> Неопределено Тогда
					...
					Если СведПрослеж <> Неопределено Тогда
						...
						Для Каждого Элемент Из СведПрослеж Цикл
							...
							СтрокаТаблицы.Количество = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "КолВЕдПрослеж");
						КонецЦикла;
				...
				СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
					ИдентификаторТовара, НоваяСтрока.ТоварНаименование, НоваяСтрока.ТоварХарактеристика, НоваяСтрока.ЕдиницаИзмеренияНаименование);
				НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
					НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
...
Процедура ПрочитатьИнформациюПродавцаУКДXDTO_5_02(ЭД, ДеревоРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Если СведТов <> Неопределено Тогда
		...
		Для Каждого Товар Из СведТов Цикл
			...
			СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
				ИдентификаторТовара, НоваяСтрока.ТоварНаименование, "", "");
			НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
				НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
		...
		Для Каждого Товар Из СведТов Цикл
			...
			Если СведПрослеж <> Неопределено Тогда
				...
				Для Каждого Элемент Из СведПрослеж Цикл
					...
					СтрокаТаблицы.НомерТовара = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "НомТовПрослеж");
					СтрокаТаблицы.ЕдиницаИзмеренияКод = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "ЕдИзмПрослеж");
					ИзмененияВКоличестве = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "ИзмКолБдПТ");
					СтрокаТаблицы.КоличествоТовараДоИзменения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ИзмененияВКоличестве, "КолТовЕдПТДо");
					СтрокаТаблицы.КоличествоТовараПослеИзменения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ИзмененияВКоличестве, "КолТовЕдПТПосле");
					СтрокаТаблицы.КоличествоТовараУвеличение = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ИзмененияВКоличестве, "КолТовЕдПТУвел");
					СтрокаТаблицы.КоличествоТовараУменьшение = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ИзмененияВКоличестве, "КолТовЕдПТУм");
				КонецЦикла;
			...
			СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
				ИдентификаторТовара, НоваяСтрока.ТоварНаименование, НоваяСтрока.ХарактеристикаОписаниеТовара, НоваяСтрока.НаименованиеЕдиницыИзмеренияДоКорректировки);
			НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
				НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
...
Процедура УдалитьПрочитатьАктОПриемкеXDTO(ЭД, ДанныеРазбора, НовыйЭД, Ошибка) Экспорт
	
	НовыйЭД.ИД = ЭД.Ид;
	НовыйЭД.ВидЭД = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НовыйЭД.ВерсияФормата = "CML_4";
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИдентификаторДокумента", ЭД.Ид);
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Номер", ЭД.Номер);
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Дата", ЭД.Дата);
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если КодВалюты <> Неопределено Тогда
			НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДанныеРазбора.ДеревоРазбора, "Валюты");
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если СвойствоСуммаОбщая <> Неопределено Тогда
			ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СуммаДокумента", ИтогоПоДокументуXDTO.Получить(СвойствоСуммаОбщая));
		КонецЕсли;
		...
		Если СвойствоЦенаВключаетНДС <> Неопределено Тогда
			ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ЦенаВключаетНДС", ИтогоПоДокументуXDTO.Получить(СвойствоЦенаВключаетНДС));
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Исходящий Тогда
			ПрочитатьДанныеКонтрагента(Исполнитель, ЭД, ДанныеРазбора, НовыйЭД, Ошибка, "Продавец");
		ИначеЕсли НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Входящий Тогда
			ПрочитатьДанныеКонтрагента(Исполнитель, ЭД, ДанныеРазбора, НовыйЭД, Ошибка, "Покупатель");
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Исходящий Тогда
			ПрочитатьДанныеКонтрагента(Заказчик, ЭД, ДанныеРазбора, НовыйЭД, Ошибка, "Покупатель");
		ИначеЕсли НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Входящий Тогда
			ПрочитатьДанныеКонтрагента(Заказчик, ЭД, ДанныеРазбора, НовыйЭД, Ошибка, "Продавец");
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если СвойствоЗаголовок <> Неопределено Тогда
			ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Заголовок", ТекстДокумента.Получить(СвойствоЗаголовок));
		КонецЕсли;
		СвойствоЗаголовок = ТекстДокумента.Свойства().Получить("ОписаниеВыполненныхРабот");
		Если СвойствоЗаголовок <> Неопределено Тогда
			ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ОписаниеВыполненныхРабот", ТекстДокумента.Получить(СвойствоЗаголовок));
		КонецЕсли;
		СвойствоЗаголовок = ТекстДокумента.Свойства().Получить("Претензии");
		Если СвойствоЗаголовок <> Неопределено Тогда
			ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Претензии", ТекстДокумента.Получить(СвойствоЗаголовок));
		КонецЕсли;
	КонецЕсли;
	
	СвойствоЭД = ЭД.Свойства().Получить("ИтогиПрописью");
	Если СвойствоЭД <> Неопределено Тогда
		ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИтогиПрописью", ЭД.Получить(СвойствоЭД));
	КонецЕсли;
	
	НаборДанных = ЭД["Товары"].Товар;
	ПрочитатьДанныеПоТЧАктаОПриемке(НаборДанных, ЭД, ДанныеРазбора, НовыйЭД, Ошибка);
...
Процедура ПрочитатьАкт501XDTO(ЭД, ДанныеРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Если Организация = Неопределено Тогда // получен Акт из не-1С системы, подставим Организацию по ИД
		...
		ОрганизацияПоИД = ДанныеУчетнойЗаписи.Организация;
			...
			Если ТипЗнч(НаборДанных) = Тип("СписокXDTO") Тогда
				Для Каждого Элемент Из НаборДанных Цикл
					...
					Если РеквизитыБазовойЕдиницыИзмерения.Свойство("Код", КодЕдИзм) Тогда
						...
						ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(КодЕдИзм),
					...
					Иначе
						...
						Если ЗначениеЗаполнено(НаимРабот) Тогда
							...
							Если Не ЗначениеЗаполнено(ИдТовара) Тогда
								ИдТовара = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(ВРег(СтрЗаменить(НаимРабот, " ", "")) + "###");
							КонецЕсли;
						КонецЕсли;
				...
				Иначе
					...
					Если Не ЗначениеЗаполнено(ИдТовара) Тогда
						ИдТовара = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(
							НаименованиеИД + "#" + НаименованиеХарактеристикиИД + "#" + ТоварКодИД + "#" + АртикулИД);
					КонецЕсли;
...
Процедура ПрочитатьСчетФактуруXDTO(ЭД, ДанныеРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Для Каждого Товар Из ЭД.Документ.ТаблСчФакт.СведТов Цикл
		...
		Если ЗначениеЗаполнено(Товар.ОКЕИ_Тов) Тогда
			ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(Товар.ОКЕИ_Тов));
	...
	Для каждого Товар Из ЭД.Документ.ТаблКСчФ.СведТов Цикл
		...
		Если НЕ Товар.Свойства().Получить("ОКЕИ_ТовДо") = Неопределено Тогда
			Если ЗначениеЗаполнено(Товар.ОКЕИ_ТовДо) Тогда
				...
				ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(Товар.ОКЕИ_ТовДо));
				Если ЗначениеЗаполнено(ЕдИзм) Тогда
		...
		Если НЕ Товар.Свойства().Получить("ОКЕИ_ТовПосле") = Неопределено Тогда
			Если ЗначениеЗаполнено(Товар.ОКЕИ_ТовПосле) Тогда
				...
				ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(Товар.ОКЕИ_ТовПосле));
				Если ЗначениеЗаполнено(ЕдИзм) Тогда
					СписокТЧ.Добавить(Строка(ЕдИзм), "ЕдиницаИзмерения");
				КонецЕсли;
			КонецЕсли;
...
Функция ЗаполнитьДанныеКомиссионераИзДопДанных(ДеревоРазбора, НовыйЭД, ПространствоИменСхемы, Ошибка)
	...
	Если ЗначениеЗаполнено(КомиссионерНаименование) Тогда
		...
		Если НовыйЭД.НаправлениеЭД = Перечисления.НаправленияЭДО.Входящий И НЕ КомиссионерДобавлен Тогда
			...
			Контрагент = ОбщегоНазначенияБЭД.НайтиСсылку(ВидКонтрагента, ИдКонтрагента, РеквизитыКонтрагента);
...
Процедура ПрочитатьПередачаТоваровПродавецXDTO(ЭД, ДанныеРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Если НаборДанных <> Неопределено Тогда
		...
		Для Каждого Элемент Из НаборДанных Цикл
			...
			СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
				ИдентификаторТовара, НоваяСтрока.НаименованиеНоменклатуры, НоваяСтрока.НаименованиеХарактеристики, НоваяСтрока.ЕдиницаИзмеренияНаименование);
			НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
				НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
		...
		Для каждого ОписаниеРабот Из СписокОписанийРабот Цикл
			...
			Если Работы <> Неопределено Тогда
				Для Каждого Элемент Из Работы Цикл
					...
					СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
						ИдентификаторТовара, Наименование, "", НоваяСтрока.ЕдиницаИзмеренияНаименование);
					НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
						НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
...
Процедура ПрочитатьАктОРасхожденияхXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка) Экспорт
	...
	Для каждого СтрокаПриемки Из РезПрием Цикл
		...
		СопоставлениеПоУмолчанию = СопоставлениеПоДаннымНоменклатуры(
			"", НоваяСтрока.Товар.Наименование, НоваяСтрока.Товар.Характеристика, НоваяСтрока.ЕдиницаИзмеренияНаименование);
		НоваяСтрока.Сопоставление = ИзвлечьСопоставлениеНоменклатурыИзДопДанныхДереваДокумента(
			НоваяСтрока.ДопДанныеПодписанные, СопоставлениеПоУмолчанию);
...
Процедура ПрочитатьДанныеПоТЧАктаОПриемке(НаборДанных, ЭД, ДанныеРазбора, НовыйЭД, Ошибка)
	...
	Для Каждого Элемент Из НаборДанных Цикл
		...
		ФорматыЭДО_CML.ПрочитатьСтрокуТоваровCML(Элемент, НовыйЭД, СписокТЧ, ПоляПоискаТовара, ДанныеРазбора, Ошибка);
		...
		Иначе
			...
			НайтиНоменклатуруПоставщикаТЧ(ПоляПоискаТовара, СписокТЧ, НовыйЭД, ДанныеРазбора, ЭД, Ошибка);
	...
	Для Каждого ТекСвойство Из Элемент.Свойства() Цикл
		...
		ИначеЕсли ВРег(ТекСвойство.Имя) = ВРег("БазоваяЕдиница") Тогда
			Если ТипЗнч(ЗнДанных) = Тип("ОбъектXDTO") Тогда
				...
				КонецЕсли;
				ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", ЗнДанных.Код, РеквизитыЕдИзм);
				НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "ЕдиницыИзмерения");
		...
		ИначеЕсли ВРег(ТекСвойство.Имя) = ВРег("ЕдиницаИзмерения") Тогда
			Если ТипЗнч(ЗнДанных) = Тип("ОбъектXDTO") Тогда
				...
				КонецЕсли;
				ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(ЗнДанных.Код), РеквизитыЕдИзм);
				НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "ЕдиницыИзмерения");
		...
		ИначеЕсли ВРег(ТекСвойство.Имя) = ВРег("Единица") Тогда
			...
			Если ЗначениеЗаполнено(ЗнДанных) Тогда
				...
				ЕдИзм = ОбщегоНазначенияБЭД.НайтиСсылку("ЕдиницыИзмерения", Строка(ЗнДанных), РеквизитыЕдИзм);
...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.Вставить("УведомлениеОбУточнении0101", "DP_UVUTOCH_1_985_00_01_02_02");
...
Функция ДвоичныеДанныеОбъектаXDTO(Знач ОбъектXDTO, Знач Кодировка = "windows-1251", Знач УказаниеТипа = Ложь)
...
Функция ОбъектXDTOДвоичныхДанных(Знач ДанныеXML, Знач ПространствоИмен, Знач ИмяТипа, Знач Кодировка = "windows-1251")
...
Процедура ПрочитатьДанныеПоРасчетномуСчетуТорг12(
	...
	БанковскийСчетСсылка = ОбщегоНазначенияБЭД.НайтиСсылку(ВидБанковскогоСчета, РеквизитыБанковскогоСчета.НомерСчета, РеквизитыБанковскогоСчета);
...
Функция СопоставлениеПоДаннымНоменклатуры(Знач Идентификатор, Знач Наименование, Знач Характеристика, Знач ЕдиницаИзмерения)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) Тогда
		Идентификатор = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(
			Наименование + "#" + Характеристика + "#" + ЕдиницаИзмерения);
	КонецЕсли;
...
Процедура НайтиНоменклатуруПоставщикаТЧ(ПоляПоискаТовара, СписокТЧ, НовыйЭД, ДанныеРазбора, ЭД, Ошибка)
	
	// Номенклатура поставщиков.
	РеквизитыНоменклатурыПоставщика = Новый Структура;
	ИмяРеквизитаВладельца = ПолучитьИмяРеквизитаВладельцаНоменклатурыПоставщиков();
	ВладелецНоменклатуры = ПолучитьРеквизитШапкиЭД(НовыйЭД, ИмяРеквизитаВладельца, ДанныеРазбора.ДеревоРазбора);
	...
	НоменклатураПоставщика = СопоставлениеНоменклатурыКонтрагентовСлужебный.НоменклатураКонтрагентаПоВладельцуИИдентификатору(
		ВладелецНоменклатуры, Ид);
		
	НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДанныеРазбора.ДеревоРазбора, "НоменклатураКонтрагентов");
	НайденнаяСтрока     = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьСтрокуВДеревеРазбора(НайденныйТипВДереве, Ид, Наименование, НоменклатураПоставщика,
															РеквизитыНоменклатурыПоставщика, ДанныеРазбора, Ошибка);
	СписокТЧ.Добавить(НайденнаяСтрока.ИндексСтроки, "НоменклатураПоставщика");
