...
Функция СохранитьКонтейнер(ОписаниеКонтейнера, КлючСинхронизации, КонтекстДиагностики, ИмяБезРасширения = "") Экспорт
	...
	ПараметрыКонтейнера = ПараметрыТранспортногоКонтейнера(ОписаниеКонтейнера.ИДДокументооборота,
		ОписаниеКонтейнера.ИДДокумента, ОписаниеКонтейнера.ИДПакета,
		ОписаниеКонтейнера.ДвоичныеДанные, КлючСинхронизации.СпособОбмена,
		КлючСинхронизации.ИдентификаторУчетнойЗаписи, КонтекстДиагностики);
...
Функция ПрисоединенныеФайлыКонтейнеров(Контейнеры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаПрисоединенныйФайл.Следующий() Цикл
			ПрисоединенныеФайлыМассив.Добавить(ВыборкаПрисоединенныйФайл.ПрисоединенныйФайл);
		КонецЦикла;
		Результат.Вставить(Выборка.ТранспортныйКонтейнер, ПрисоединенныеФайлыМассив);
	КонецЦикла;
	УстановитьПривилегированныйРежим(Ложь);
	...
	Если ДополнительныеПараметры.Свойство("ParentDocument") Тогда
		Если ТипЗнч(ДополнительныеПараметры.ParentDocument) = Тип("Массив") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Карточка.ИдентификаторыСообщенийОснований,
				ДополнительныеПараметры.ParentDocument);
		Иначе
	...
	Если ДополнительныеПараметры.Свойство("LinkedDocument") Тогда
		Если ТипЗнч(ДополнительныеПараметры.LinkedDocument) = Тип("Массив") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Карточка.ИдентификаторыДокументооборотовОснований,
				ДополнительныеПараметры.LinkedDocument);
		Иначе
	...
	Для Каждого Документооборот Из МетаданныеXDTO.DocFlow Цикл
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			Если Документ.Files.MainImage <> Неопределено Тогда
				...
				Попытка
					ДвоичныеДанныеФайлаЭД = Новый ДвоичныеДанные(СтруктурированноеИмяФайла.ПолноеИмя);
				Исключение
					...
					СтруктурированноеИмяФайла = СтруктурированноеПолноеИмяФайла(КаталогРаспаковки, ПолноеИмяФайла);
					ДвоичныеДанныеФайлаЭД = Новый ДвоичныеДанные(СтруктурированноеИмяФайла.ПолноеИмя);
				...
				ОписаниеДокументаКонтейнера.ОписаниеДанных.ДвоичныеДанные = Новый ДвоичныеДанные(
...
//
// Параметры:
//  ПараметрыКонтейнера - см. ПараметрыТранспортногоКонтейнера
...
Функция ЗаполнитьТранспортныйКонтейнер(ПараметрыКонтейнера, ТранспортныйКонтейнер = Неопределено)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ПараметрыКонтейнера.Контрагент)
			Или Не ЗначениеЗаполнено(ПараметрыКонтейнера.Организация)
			Или Не ЗначениеЗаполнено(ПараметрыКонтейнера.ДоговорКонтрагента) Тогда
...
Функция СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики = Неопределено)
	...
	Для Каждого ПрисоединенныйФайлКонтейнера Из ПрисоединенныеФайлыКонтейнеров Цикл
		ПрисоединенныеФайлыМассив.Добавить(ПрисоединенныйФайлКонтейнера.Значение[0]);
	КонецЦикла;
		...
		Попытка
			...
			Если (ЗначенияРеквизитовКонтейнера.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный
				...
				ПараметрыКонтейнера = ПараметрыТранспортногоКонтейнера(
					...
					ДвоичныеДанныеФайла, ЗначенияРеквизитовКонтейнера.СпособОбменаЭД,
					ЗначенияРеквизитовКонтейнера.Получатель, КонтекстДиагностики);
