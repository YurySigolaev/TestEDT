...
// 	ПараметрыАвторизации - см. НовыеПараметрыАвторизации
// Возвращаемое значение:
// 	Булево - Истина - если регистрационный пакет отправлен успешно
Функция ОтправитьРегистрационныйПакет1СЭДО(ДанныеПакета, КонтекстДиагностики = Неопределено,
	...
	Ответ.Вставить("ЕстьОшибки", Не Результат.Успех);
...
Функция ПрочитатьОписаниеОшибкиОператора(Знач Ответ, Знач ТекстОшибкиПоУмолчанию = "")
	...
	Попытка
		...
		Ошибка.ИстекСрокДействияМаркера = Ошибка.Код = 1201;
	Исключение
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора)
	...
	// Получение признака подключения организации через услугу Onboarding.
	ПодключенПоOnboarding = ОбщегоНазначенияБЭД.БазаСозданаЧерезOnboarding();
