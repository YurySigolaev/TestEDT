...
Функция МаршрутПоВсемЗависимымНастройкамВалиден(Параметры) Экспорт
	...
	Если ЕстьОбменСКонтрагентами ИЛИ ЕстьОбменСБанками Тогда
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			Если Не ЕстьОбменСБанками Тогда
				ТекстЗапросаОбменСКонтрагентами = СтрЗаменить(ТекстЗапросаОбменСКонтрагентами,
					"ПОМЕСТИТЬ ВидыДокументовПоНастройкам", "");
			КонецЕсли;
				...
				Пока ВыборкаПолучателей.Следующий() Цикл
					...
					Пока ВыборкаДоговоров.Следующий() Цикл
						...
						Иначе 
							...
							КлючНастроек = НастройкиЭДО.НовоеОписаниеПолейКлючаНастройкиОтправки();
							ЗаполнитьЗначенияСвойств(КлючНастроек, ОписаниеНастройки);
							ПредставлениеНастройки = ПредставлениеНастройкиОтправкиЭлектронныхДокументов(КлючНастроек);
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			Отбор = НастройкиЭДО.НовыйОтборНастроекВнутреннегоЭДО();
			Отбор.ПроизвольноеУсловие = "МаршрутПодписания = &МаршрутПодписания И ВидПодписи <> &ВидПодписи";
			ЗапросНастроек = НастройкиЭДО.ЗапросНастроекВнутреннегоЭДО("НастройкиВнутреннегоЭДО", Отбор);
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Пока ВыборкаОрганизация.Следующий() Цикл
					...
					Если ВыборкаДетальныеЗаписи.Следующий() Тогда
						ЗначенияКлюча = НастройкиЭДОКлиентСервер.НовоеОписаниеПолейКлючаНастройкиВнутреннегоЭДО();
						ЗначенияКлюча.Организация = ВыборкаОрганизация.Организация;
						ЗначенияКлюча.ВидДокумента = ВыборкаДетальныеЗаписи.ВидДокумента;
						КлючОбъекта = НастройкиЭДО.КлючОбъектаНастроекВнутреннегоЭДО(ЗначенияКлюча);
...
// Возвращает описание вида ошибки маршрута подпсания
//  
// Возвращаемое значение:
...
Функция ЗапросТекущегоСостоянияПодписания(ИмяВременнойТаблицы) Экспорт
	...
	ШаблонЗапроса =
		...
		|	РегистрСведений.СостояниеПодписанияЭД КАК СостояниеПодписанияЭД
		|ГДЕ
		|	СостояниеПодписанияЭД.Текущий = ИСТИНА";
...
Процедура ЗаполнитьНачальныеНастройкиМаршрутов() Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр(
			"ru = 'Заполнение начальных настроек маршрутов'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
