...
Процедура ОбновитьОписаниеФорматовДляПреобразования(КонтекстДиагностики, ПараметрыНачальногоЗаполения = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыНачальногоЗаполения) Тогда
		ДанныеФайла = Новый ДвоичныеДанные(ПараметрыНачальногоЗаполения.Каталог + "supported_formats_v20.xml");
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если НайденныйФормат = Неопределено
			...
			Если ЗначениеЗаполнено(ПараметрыНачальногоЗаполения) Тогда
				...
				ПараметрыПравила.Вставить("ДатаЗагрузки", Дата(ЧтениеXML.ЗначениеАтрибута("upd"))); 
				
				СсылкаНаРесурс = ПараметрыНачальногоЗаполения.Каталог + МенеджерЗаписи.СсылкаНаРесурс; 
				
				ОбновитьПравилоПреобразования(СсылкаНаРесурс, ПараметрыПравила, КонтекстДиагностики, ПараметрыНачальногоЗаполения);
	...
	Если Не ЗначениеЗаполнено(ПараметрыНачальногоЗаполения) Тогда		
		Константы.ДатаЗагрузкиОписанияФорматовДляПреобразования.Установить(ТекущаяУниверсальнаяДата);
	КонецЕсли;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если  НачальноеЗаполнение 
		  Или ПараметрыПравила.ВариантЗаполнения      = ЧтениеXML.ЗначениеАтрибута("type")
			...
			Если ПараметрыПравила.ДатаЗагрузки > Дата(ЧтениеXML.ЗначениеАтрибута("upd")) Тогда
