...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Он говорил на том изысканном французском языке, на котором не только говорили, но и думали наши деды, и с теми,
	//  тихими, покровительственными интонациями, которые свойственны состаревшемуся в свете и при дворе значительному 
	// человеку. Он подошел к Анне Павловне, поцеловал ее руку, подставив ей свою надушенную и сияющую лысину, и покойно 
	// уселся на диване.
...
Процедура СтороныПередНачаломИзменения(Элемент, Отказ)
	...
	МожноКорректироватьПодписан = 
		СостояниеПоЭДО = ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭДДО.НеСформирован")
		Или (ДоступнаКорректировкаЭД
			И (СостояниеПоЭДО = ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭДДО.ОтклоненПолучателем")
				Или СостояниеПоЭДО = ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭДДО.ТребуетсяУточнение")));
...
Процедура ПодписатьДокументЗавершение(Результат, Параметры) Экспорт
	...
	Если Не НаправлениеИсходящий Тогда
		...
		ОбменСКонтрагентамиДОСлужебныйКлиент.ПодписатьУтвердитьЭДНаСторонеПолучателя(Объект.Ссылка, ОбработчикЗавершения);
...
Процедура ВыполнитьДействиеПоЭДОПослеПроверкиСертификатов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИмяКоманды = "ПодписатьДокументЭДО" Тогда
		...
		ПодписатьДокумент(Истина, Результат);
	...
	ИначеЕсли ИмяКоманды = "ОтклонитьДокументЭДО" Тогда
		...
		ОбменСКонтрагентамиДОСлужебныйКлиент.ОтклонитьДокументПоЭДО(Объект.Ссылка, ОписаниеОповещения);
	...
	Если ПодписанЭДО Тогда
		...
		ИначеЕсли СостояниеПоЭДО = Перечисления.СостоянияВерсийЭДДО.НеСформирован Или (ДоступнаКорректировкаЭД
			И (СостояниеПоЭДО = Перечисления.СостоянияВерсийЭДДО.ОтклоненПолучателем
				Или СостояниеПоЭДО = Перечисления.СостоянияВерсийЭДДО.ТребуетсяУточнение)) Тогда
...
Процедура УстановитьВидимостьПоЭДО()
	...
	ЭлементыДействийЭДО.Вставить(
		Перечисления.ДействияПоЭДО.Утвердить, Элементы.ПринятьДокументЭДО);
	
	ЭлементыОтклонения = Новый Массив;
	ЭлементыОтклонения.Добавить(Элементы.ОтклонитьДокументЭДО);
	ЭлементыОтклонения.Добавить(Элементы.КомандаОтклонитьЭДО);
	
	ЭлементыДействийЭДО.Вставить(
		Перечисления.ДействияПоЭДО.Отклонить, ЭлементыОтклонения);
	
	ЭлементыДействийЭДО.Вставить(
		Перечисления.ДействияПоЭДО.Сформировать, Элементы.ПодписатьДокументЭДО);
	
	ЭлементыАннулирования = Новый Массив;
	ЭлементыАннулирования.Добавить(Элементы.АннулироватьДокументЭДО);
	ЭлементыАннулирования.Добавить(Элементы.КомандаАннулироватьЭДО);
	
	ЭлементыДействийЭДО.Вставить(
		Перечисления.ДействияПоЭДО.Аннулировать, ЭлементыАннулирования);
	...
	ОбменСКонтрагентамиДОСлужебный.УстановитьВидимостьДоступностьКомандЭДО(
		Объект.Ссылка, ЭтотОбъект, ЭлементыДействийЭДО);
